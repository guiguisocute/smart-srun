...
failover_enabled = s:option(Flag, "failover_enabled", "夜间模式自动切换热点SSID")
bind_flag(failover_enabled, "failover_enabled")

hotspot_ssid = s:option(Value, "hotspot_ssid", "热点SSID", "手动输入要切换的热点 SSID")
...
hotspot_encryption = s:option(ListValue, "hotspot_encryption", "热点加密方式")
...
hotspot_key = s:option(Value, "hotspot_key", "热点密码")
...
quiet_hours_enabled = s:option(Flag, "quiet_hours_enabled", "按时段自动上/下线", quiet_desc)
bind_flag(quiet_hours_enabled, "quiet_hours_enabled")

quiet_start = s:option(Value, "quiet_start", "下线时间（北京时间 HH:MM）")
...
quiet_end = s:option(Value, "quiet_end", "上线时间（北京时间 HH:MM）")
...
force_logout_in_quiet = s:option(Flag, "force_logout_in_quiet", "进入下线时段时强制下线")
bind_flag(force_logout_in_quiet, "force_logout_in_quiet")

developer_mode = s:option(Flag, "developer_mode", "开发者选项")
bind_flag(developer_mode, "developer_mode")

backoff_enable = s:option(Flag, "backoff_enable", "登录失败时启用退避重试")
bind_flag(backoff_enable, "backoff_enable")

backoff_max_retries = s:option(Value, "backoff_max_retries", "最大重试次数（0 为无限）")
backoff_max_retries.datatype = "uinteger"
bind_text(backoff_max_retries, "backoff_max_retries")

backoff_initial_duration = s:option(Value, "backoff_initial_duration", "初始等待时长（秒）")
...
backoff_max_duration = s:option(Value, "backoff_max_duration", "最大等待时长（秒）")
...
backoff_exponent_factor = s:option(Value, "backoff_exponent_factor", "指数因子")
...
backoff_inter_const_factor = s:option(Value, "backoff_inter_const_factor", "内常数因子（秒）")
...
backoff_outer_const_factor = s:option(Value, "backoff_outer_const_factor", "外常数因子（秒）")
...
if cfg.developer_mode == "1" then
    switch_hotspot_test = s:option(Button, "_switch_hotspot_test", "测试切到热点")
    ...
    switch_campus_test = s:option(Button, "_switch_campus_test", "测试切回校园网")
...
log_text = s:option(DummyValue, "_log_text", "运行日志")
